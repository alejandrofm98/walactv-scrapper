...
NOISE_SUFFIXES = [
    ...
    r'\bmulti[-\s]?sub\b', r'\bno\s+sub\b', r'\bfrench\s+only\b',
    r'\bfrench\s+quebec\b', r'\bquebec\b',
    r'\beng[-\s]?sub\b', r'\bwith\s+sub\b',
    r'\bjason\s+statham\b', r'\bharvey\s+keitel\b', r'\bliam\s+neeson\b',
...
def build_series_key(serie_name: Optional[str], nombre: Optional[str] = None) -> Optional[str]:
