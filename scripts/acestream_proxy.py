  ...
  try:
    ...
    if rewrite_manifest or is_manifest_content(content_type, target_url):
      try:
        ...
        urls_replaced = original_content.count(ACESTREAM_BASE)
  ...
  try:
    ...
    if resp2.status_code in [301, 302, 303, 307, 308]:
      ...
      if location:
        ...
        if content:
          rewritten = content.replace(ACESTREAM_BASE,
                                      PUBLIC_DOMAIN)
...
def test_manifest(id_content):
  ...
  try:
    ...
    if resp.status_code == 200:
      ...
      # Reescribir
      content = content.replace(ACESTREAM_BASE, PUBLIC_DOMAIN)

      return {
        "status": "success",
        "original_preview": original[:300],
        "rewritten_preview": content[:300],
        "urls_replaced": original.count(ACESTREAM_BASE),
