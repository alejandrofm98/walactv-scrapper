...
def get_or_prewarm_stream(stream_id, wait=True):
    ...
    if wait:
        ...
        logger.warning(f"⏱️ Warmup timeout")
        return None
...
def analyze_ts_chunk_deep(chunk_data):
    ...
    try:
        ...
        audio_found = {}
        aac_count = chunk_data.count(b'\xff\xf1') + chunk_data.count(b'\xff\xf9')
        if aac_count > 0:
            audio_found["AAC"] = aac_count
        ac3_count = chunk_data.count(b'\x0b\x77')
        if ac3_count > 0:
            audio_found["AC3"] = ac3_count
        mp3_count = sum(chunk_data.count(p) for p in [b'\xff\xfb', b'\xff\xfa', b'\xff\xf3', b'\xff\xf2'])
        if mp3_count > 0:
            audio_found["MP3"] = mp3_count
        dts_count = chunk_data.count(b'\x7f\xfe\x80\x01') + chunk_data.count(b'\xfe\x7f\x01\x80')
        if dts_count > 0:
...
def analyze_pmt_table(chunk_data):
    pmt_info = {"found": False, "video_stream_type": None, "audio_stream_types": [], "raw_stream_types": []}
    try:
            ...
            if chunk_data[i] == 0x47:
                ...
                if pid in [0x1000, 0x0100, 0x1FFF]:
                    ...
                    if chunk_data[i + 1] & 0x40:
                        ...
                        if table_start < len(chunk_data) and chunk_data[table_start] == 0x02:
                            pmt_info["found"] = True
                            for j in range(table_start + 10, min(table_start + 100, len(chunk_data))):
...
def detect_raw_patterns(chunk_data):
    ...
    patterns["aac_markers"] = chunk_data.count(b'\xff\xf1') + chunk_data.count(b'\xff\xf9')
...
def consolidate_codec_analysis(codec_list):
    ...
    video_profile = max(video_profiles, key=video_profiles.get) if video_profiles else None
    video_level = max(video_levels, key=video_levels.get) if video_levels else None
...
def is_chromecast_compatible_v2(codec_info):
    ...
    logger.info(f"   Detecciones: H264={video_detection.get('h264_detections', 0)}, H265={video_detection.get('h265_detections', 0)}")
    ...
    if "h.265" in video_lower or "hevc" in video_lower or h265_detections > 0:
        ...
        if hevc_profile_info:
            ...
            logger.info(f"     📊 HEVC Details: {profile_name} / {tier} Tier / Level {level}")
            ...
            try:
                ...
                pass
            if tier == "High":
                logger.info(f"  ⚠️ HEVC High Tier - Puede causar problemas en algunos Chromecasts")
            if "Main" in profile_name and "10" not in profile_name:
                logger.info(f"  ✅ HEVC Main Profile, Level {level} - Compatible con Chromecast Ultra/Gen3+")
            ...
            if "unknown" in profile_name.lower():
                logger.info(f"  ⚠️ HEVC con profile desconocido - Asumiendo compatible conservadoramente")
                return True
        if h265_detections > 5:
            logger.info(f"  ⚠️ HEVC detectado ({h265_detections} patrones) sin info de profile")
            logger.info(f"     Puede ser compatible con Chromecast Ultra/Gen3+ (HEVC Main)")
            logger.info(f"     Pero incompatible con Chromecast Gen 1/2 (sin HEVC)")
            return True
        if h265_detections <= 3:
            logger.info(f"  ⚠️ Pocas detecciones HEVC ({h265_detections}) - Stream posiblemente inestable")
            return None

    if h264_detections > 0 and (h265_detections / h264_detections) > 0.5:
        logger.info(f"  ❌ Alto ratio H265/H264 ({h265_detections}/{h264_detections}) - Probablemente HEVC")
        return False
    ...
    if "h.264" in video_lower or "avc" in video_lower:
        ...
        if video_level:
            try:
                ...
                if level_float > 5.2:
                    logger.info(f"  ⚠️ Level H.264 muy alto: {video_level} (puede causar problemas)")
            except:
                pass
        logger.info(f"  ✅ H.264 compatible ({video_profile or 'unknown profile'}, level {video_level or 'unknown'})")
        audio_codecs = codec_info.get("audio_codecs", [])
        has_aac = any("AAC" in c.upper() for c in audio_codecs)
        has_only_incompatible = all(any(bad in c.upper() for bad in ["AC3", "E-AC3", "DTS"]) for c in audio_codecs)
...
def analyze_stream_deep(id_content):
    ...
    try:
        ...
        chunk_urls = re.findall(r'(http://acestream-arm:6878/ace/c/[^\s]+\.ts)', manifest_content)
        if not chunk_urls:
            result["status"] = "error"
        ...
        result["manifest_info"]["chunks_preview"] = [url.split('/')[-1] for url in chunk_urls[:5]]
        ...
        for idx, chunk_url in enumerate(chunk_urls[:chunks_to_test]):
            try:
                ...
                logger.info(f"     ✓ {len(chunk_data)} bytes, Video: {analysis['deep_analysis'].get('video_codec')}, Audio: {analysis['deep_analysis'].get('audio_codecs')}")
            except Exception as e:
                logger.warning(f"  ✗ Error chunk {idx}: {e}")
                chunk_details.append({"chunk_name": chunk_url.split('/')[-1], "error": str(e)})
...
def compare_streams(id1, id2):
    ...
    return {
        "stream_1": {"id": id1, "analysis": analysis1},
        "stream_2": {"id": id2, "analysis": analysis2},
        "differences": {
            "video_codec": {
...
def channel_status(id_content):
    try:
        ...
        result = {"id": id_content, "status": "unknown", "chromecast_compatible": None, "codec_info": None}
        if resp.status_code == 302:
            ...
            try:
                manifest_url = f"{ACESTREAM_BASE}/ace/manifest.m3u8?id={id_content}"
                manifest_resp = requests.get(manifest_url, allow_redirects=True, timeout=30)
                if manifest_resp.status_code == 200:
                    manifest_content = manifest_resp.text
                    chunk_urls = re.findall(r'(http://acestream-arm:6878/ace/c/[^\s]+\.ts)', manifest_content)
                    if chunk_urls:
                        chunks_to_analyze = chunk_urls[:min(5, len(chunk_urls))]
                        logger.info(f"🔍 Analizando {len(chunks_to_analyze)} chunks para detección robusta")
                        all_codecs = []
                        for idx, chunk_url in enumerate(chunks_to_analyze):
                            try:
                                logger.info(f"  └─ Chunk {idx + 1}/{len(chunks_to_analyze)}: {chunk_url[-40:]}")
                                ...
                                logger.info(f"     ✓ Video: {codec_info.get('video_codec')}, Audio: {codec_info.get('audio_codecs')}")
                            except Exception as e:
                                logger.warning(f"⚠️ Error analizando chunk {idx + 1}: {e}")
                                continue
                        logger.info(f"✅ Total chunks analizados exitosamente: {len(all_codecs)}")
                        consolidated = consolidate_codec_analysis(all_codecs)
                        result["codec_info"] = consolidated
                        result["chunks_analyzed"] = len(all_codecs)
                        if consolidated.get("video_codec"):
                            result["chromecast_compatible"] = is_chromecast_compatible_v2(consolidated)
...
def debug_manifest(id_content):
    ...
    try:
        ...
        results["steps"].append({
            "step": "getstream", "url": url1, "status": resp1.status_code, "elapsed_seconds": round(elapsed1, 2),
            "headers": dict(resp1.headers), "location": resp1.headers.get('Location', 'N/A')
        })
        ...
        results["steps"].append({
            "step": "manifest_direct", "url": url2, "status": resp2.status_code, "elapsed_seconds": round(elapsed2, 2),
            "headers": dict(resp2.headers), "location": resp2.headers.get('Location', 'N/A'),
        ...
        if resp2.status_code in [301, 302, 303, 307, 308]:
            ...
            if location:
                ...
                results["steps"].append({
                    "step": "follow_redirect", "url": url3, "status": resp3.status_code, "elapsed_seconds": round(elapsed3, 2),
                    "headers": dict(resp3.headers), "content_preview": content[:500] if content else None
                })
                if content:
                    rewritten = content.replace('http://acestream-arm:6878', PUBLIC_DOMAIN)
                    results["rewritten_preview"] = rewritten[:500]
                    results["urls_in_manifest"] = content.count('http://acestream-arm:6878')

        try:
            resp4 = requests.get(f"{ACESTREAM_BASE}/webui/api/service?method=get_stats", timeout=5)
            results["engine_stats"] = resp4.json() if resp4.status_code == 200 else None
        except:
...
def test_manifest(id_content):
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            return {"status": "error", "code": resp.status_code, "response": resp.text[:500]}
    except Exception as e:
...
def manifest_query():
    ...
    if is_chromecast:
        ...
        # Intentar usar stream pre-calentado
        warmup = get_or_prewarm_stream(id_content, wait=True)

        if not warmup or not warmup.ready:
            logger.warning(f"  ⚠️ Stream not pre-warmed, using fallback")
            # Fallback: activar directamente
            try:
                requests.get(f"{ACESTREAM_BASE}/ace/getstream?id={id_content}",
...
def manifest_chromecast():
    ...
    return redirect(f"{PUBLIC_DOMAIN}/ace/manifest.m3u8?id={id_content}&chromecast=1", code=307)
...
def getstream_query():
    ...
    # Si es Chromecast, pre-calentar en segundo plano
    if is_chromecast:
        Thread(target=prewarm_stream, args=(id_content,), daemon=True).start()
